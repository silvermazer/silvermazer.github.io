...
document.addEventListener("keyup", function(){// remove crouch and let button be pressed again after hold
	...
	if(event.key.toLowerCase() == "s" && redcrouch == true){if(redhit == 0){redcrouch = false;}}
...
function loop(){
	...
	ctx.fillRect(0, 0, cnv.width, 300);
	...
	if(redy < 300){redmoveY = Math.abs(redmoveY); redhitstun = 0;}
	...
	if(bluey < 300){bluemoveY = Math.abs(bluemoveY); bluehitstun = 0;}
	//movement
	if(blueonground == false){
		if(bluemoveY < -2){
			bluey += bluemoveY;
